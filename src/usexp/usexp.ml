...
module Atom = struct
  ...
  let length (A s) = String.length s

  let compare (A x) (A y) = String.compare x y

  let sub (A s) ~pos ~len = A(String.sub s ~pos ~len)
  let ( ^ ) (A x) (A y) = A(x ^ y)

  let chop_extension (A s) = A(Filename.chop_extension s)

  [@@@warning "-3"]
  let capitalize (A s) = A(String.capitalize s)
