...
let sexp_of_var_syntax = function
  | Parens -> Sexp.(Atom(Atom.unsafe_of_string "parens"))
  | Braces -> Sexp.(Atom(Atom.unsafe_of_string "braces"))

let sexp_of_item =
  let open Sexp in function
    | Text s -> List [Atom (Atom.unsafe_of_string "text") ; To_sexp.string s]
    | Var (vs, s) -> List [sexp_of_var_syntax vs ; atom_or_quoted_string s]
